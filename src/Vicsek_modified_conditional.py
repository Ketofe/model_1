...
#It is like the regular vicsek_modified but it stops when either defectors or cooperators have won.



                                                         #Here one can set initial conditions if not they wil be set to default values further down
def Vicsek_modified_conditional(v_0,eta,r, N,L,iterations,alpha,beta,initial_pos=None,initial_theta=None,initial_s=None):
        ...
        for iteration in range(iterations-1):
                ...
                #Condtion for steady state
                if sum(s[iteration])/N==0 or sum(s[iteration])/N==1:
                       #Removing the zeros that would be at the end.
                       pos=pos[:iteration+1]
                       theta=theta[:iteration+1]
                       s=s[:iteration+1]
                       break
